...
app.post('/regattas', async(req, res) => {
  ...
  try {
    ...
    const data = await client.query(`
    INSERT INTO regattas(name, type, city, length_km, recommend)
    VALUES($1, $2, $3, $4, $5)
    RETURNING *
...
app.put('/regattas/:id', async(req, res) => {
  ...
  try {
    ...
    const data = await client.query(`
    UPDATE regattas
      set name=$1, type=$2, city=$3, length_km=$4, recommend=$5
      WHERE regattas.id=$6
    RETURNING *
    `, [newRegatta.name, newRegatta.type, newRegatta.city, newRegatta.length_km, newRegatta.recommend, regattaId]);
